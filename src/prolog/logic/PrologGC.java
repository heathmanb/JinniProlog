...
import static prolog.logic.Defs.isATOMIC;
import static prolog.logic.Defs.isCOMPOUND;
...
final class PrologGC {
    ...
    /**
     * The constructor gets a reference to a machine's registers choice point
     * stack, trail and heap. Various methods defined in this class will perform
     * GC - by possibly updating
     */
  ...
  //private final static boolean profile=true;
    final void startGCtimer() {
        oneGCtime = currentTimeMillis();
    }

    final void endGCtimer() {
        oneGCtime = currentTimeMillis() - oneGCtime;
        gcTIME += oneGCtime;
    }
  ...
  // a subset of recongnizers - should be all the GC needs
    ...
    final static boolean isAtomic(int cell) {
        return isATOMIC(cell);
    }

    /**
     * check if the cell starts a compound term - use getArity(cell) to see how
     * many cells follow this
     */
    final static boolean isCompound(int cell) {
        return isCOMPOUND(cell);
    }

    /**
     * returns the arity of a functor cell indicating the beginning of a
     * compound term
     */
    ...
    public boolean collect() throws SystemException {
        ...
        if (trace >= 2) {
            dump("running gc, heap used=" + before);
        }
        if (trace == 3) {
            dumpChoice();
            dumpRegs();
        ...
        if (trace == 3) {
            dump(Interact.NL + "TOTAL MARKED=" + mark_ctr + "/" + heap.getHeapTop());
            dumpMarkedHeap(30, true);
        }
        sweep(); // sweep and copy to area above top then relocate back and update stacks
        if (trace == 3) {
            dump(Interact.NL + "======================:" + mark_ctr + "/" + heap.getUsed());
            dumpChoice();
            dumpRegs();
            dumpTrail(20);
            dumpMarkedHeap(30, false);
            dump(Interact.NL + "TOTAL MARKED=" + mark_ctr + "/" + heap.getUsed());
        }
    ...
    int mark() throws SystemException {
        ...
        if (trace >= 2) {
            dump("mark_ctr before marking tops: " + ctr);
        }
    ...
    // further marking:
        ...
        if (trace >= 2) {
            dump("mark_ctr after marking tops: " + ctr);
        }
    ...
    void checkUpperHeap() {
        if (mark_ctr >= heap.getFree()) {
            if (trace > 0) {
                dump("GC: marked=" + mark_ctr + "free=" + heap.getFree());
            }
            heap.expand(); // make sure we have room
            if (trace > 0) {
                dump("GC: expanded, now free=" + heap.getFree());
            }
        }
    }

    /**
     * Copies each marked cell to the upper heap and set forward links from old
     * heap cells (vars and nonvars!) to copied new cells note: variables in new
     * cells still point to old addresses therefore on more pass is needed to
     * fix them - see relocateHeap(...)
     */
    int liftMarked() {
     ...
     * Note: the heap still needs to be slided - but this is delayed until
     * forwarding pointers in old cells are not be needed anymore.
     */
    ...
    /**
     * Relocates choicePoints (heapTops and regs) using lower heap as a fast
     * relocation table.
     */
    ...
    /**
     * Relocates registers in regs[1..arity] using lower heap as a fast
     * relocation table.
     */
    void relocateRegs() {
    ...
    /**
     * Relocates trail and sets all dead trail cell to point to dummy unbound
     * variable at address 0 This makes relocating the to of the trail pointers
    ...
    void relocateTrail(int min, int max) {
        for (int i = min; i <= max; ++i) {
            ...
            if (!marked[h]) {
                ...
                trail.update(i, slideBack(h));
            }
        }
    }

    /**
     * Slides heap from upper to lower area. This should be last thing to do -
     * as relocation table in lower heap will be overwritten.
     */
    ...
    void dumpMarkedHeap(int max, boolean onlyMarked) throws SystemException {
        ...
        for (int i = 0; ctr < max && i < heap.getUsed(); i++) {
            ...
            String m = (!marked[i]) ? " " : "*";
            dump("[" + ctr + "] " + m + "_" + i + "=>" + heap.cellToString(i));
            ++ctr;
        }
    }
    ...
    void dumpRegs() throws SystemException {
        dump(Interact.NL + "REGS: " + arity);
        for (int i = 1; i <= arity; i++) {
            dump("[" + i + "] " + heap.dumpCell(regs[i]));
        }
    }
    ...
    static void dumpTrail(TrailStack trail, HeapStack heap, int max) {
        dump(Interact.NL + "TRAIL: " + trail.getTop());
        if (0 == max) {
            max = trail.getTop();
        }
        for (int i = 0; i < max && i <= trail.getTop(); ++i) {
            dump("[" + i + "] " + trail.at(i) + "=>" + heap.cellToString(trail.at(i)));
        }
    }
}
