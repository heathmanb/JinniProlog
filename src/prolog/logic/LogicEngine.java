...
import static prolog.logic.CodeStore.GETINSTR_OP;
import static prolog.logic.CodeStore.getInstructionName;
import static prolog.logic.Interact.add_to_path;
import static prolog.logic.Interact.clear_path;
import static prolog.logic.Interact.del_from_path;
import static prolog.logic.Interact.errmes;
import static prolog.logic.Interact.fatal_error;
import static prolog.logic.Interact.halt;
import static prolog.logic.Interact.path_element;
import static prolog.logic.Interact.push_to_path;
import static prolog.logic.Interact.warnmes;
import static prolog.logic.Prolog.cloneOrigProlog;
...
public class LogicEngine extends HeapStack implements Runnable {
     ...
     *
     * @param i Index of the temporary register in the range 1 to TEMPREGS.
     * @return
     *
     */
    ...
    private boolean putArg(int i, int x) {
    ...
    private void putLastArg(int i, int x) {
    ...
    private void INIT_INTERP(Object query) throws PrologException {
        choice.clear();
        trail.clear();
        clear();
        S = 0;
    ...
    private int queryTerm(Object query) throws PrologException {
        if (null == query) {
    ...
    private int runInternal() throws PrologException {

        for (;;) {

            curInstr = codeStore.GETINSTR(instrPtr, 0);
            op = GETINSTR_OP(curInstr);
            ...
            switch (op) {
                ...
                case NEW_CODE_2: {
                    ...
                    Prolog other = cloneOrigProlog(getAtomName(xval));
    ...
    private boolean gc(int pred, int arity) throws PrologException {
         ...
         * call your gc here !!!
         */
        final PrologGC collector
                = new PrologGC(arity, regs, choice, trail, (HeapStack) this);
    ...
    private boolean gc_call() throws PrologException {
        ...
        int _arity = GETARITY(pred);
        gc_trace("BEFORE:", pred, _arity);
        boolean ok = gc(pred, _arity);
        gc_trace("AFTER: ", pred, _arity);
        return ok;
    }

    final void apply(int fun) {
        int _arity = GETARITY(fun) - 1;
        fun = PUTARITY(fun, _arity);
        pushTerm(fun);
        int _xref = getHeapTop();
        // COPY_CELLS(xref,regs,arity);
        pushCells(regs, 1, _arity);
        // for(int i=1; i<=arity; i++) pushTerm(regs[i]);
        regs[2] = regs[_arity + 1];
        regs[1] = _xref;
        // Prolog.dump("APPLY: "+termToString(xref));
    }

    final boolean isCompiled(int PredArity) throws PrologException {
        int _instrPtr = dict.getpred(PredArity);
        return !dict.do_isEmpty(_instrPtr);
    }
    ...
    public static void warn_mes(String mes) {
        warnmes(Interact.NL + "Warning: " + mes + ".");
    }
    ...
    private int float_op(int op, int x, int y) throws TypeException {
        double fx;
        if (isINTEGER(x)) {
            fx = OUTPUT_INT(x);
        } else {
        ...
        switch (op) {
            ...
            default:
                throw new TypeException("bad data in arithmetic operation: "
                        + getInstructionName(op) + "(" + termToString(x) + ","
                        + termToString(y) + ")");
        }
    ...
    private void COMPUTE(int operator) throws TypeException,
            ExistenceException {
        int i1 = X(1);
        int i2 = X(2);
    ...
    private void MUST_BE(int relop) throws TypeException {
        ires = X(1);
        xval = X(2);
    ...
    private void IN(int I, int Expr) {
        xref = Expr;
        if (isNONVAR(xref)) {
            setX(I, xref);
        } else {
    ...
    private String toCanonical(int x) {
        String s;
        if (isVAR(x)) {
            x = getRef(x);
        }
    ...
    private void add_instr() throws PrologException {
        String name;
        int xval = X(4);
        if (isINTEGER(xval)) {
    ...
    final public Object query_engine(Object query) {
        ...
        try {
            ...
            if (!isStopped()) {
                errmes("error in machine.query_engine() on query=" + query, e);
            }
        }
        if (null == answer) {
            answer = "no";
    ...
    final public boolean load_engine(Object query) {
        ...
        try {
            ...
            errmes("error in machine.load_engine():" + this, e);
            stop();
            return false;
        }
    }
    ...
    final public int ask() throws PrologException {
        ...
        try {
            ...
            if (!isStopped()) {
                ...
                errmes("error in engine " + this + ":ask()=>" + s + ": ", e);
    ...
    private int compVal(int xval) throws TypeException {
    ...
    private int ocompare(int l, int vl, int r, int vr) throws TypeException {
    ...
    private int toReal(int i) {
        double d = OUTPUT_INT(i);
        int r = termReader.putFloat(d);
        // Prolog.dump("toReal:"+termToString(i)+"==>"+s+"==>"+refToDouble(r));
        return r;
    }

    private double refToDouble(int xref) throws TypeException {
        ...
        if (xval == prolog.G_FLOAT) { // $float
            ...
            deref(xref + 3);
            int res = this.xval;
            return toDouble(i1, i2, res);
        } else {
            throw new TypeException("number expected in refToDouble, got: " + termToString(xref));
        }
    }

    private int compareReals(int l, int r) throws TypeException {
    ...
    private int compare(int l, int r) throws TypeException {
        int ires;
        int vl, vr;
        if (isVAR(l)) {
            deref(l);
    ...
    private boolean float_rel(int relop, int x, int y) throws TypeException {
    ...
    public int jcall(int X1, int X2) throws PrologException {
        ...
        try {
            if (null != f) {
                switch (f) {
                    case "get_prolog_class":
                        Prolog P = getPrologClass(x);
                        if (null == P) {
                            r = prolog.S_null;
                        } else {
                            r = "" + addObject(P);
                    ...
                    break;
                    case "path_element":
                        r = path_element(parseInt(x));
                        break;
                    case "advance_code_top": {
                        prolog.advance_code_top();
                    }
                    break;
                    case "rollback": {
    ...
    synchronized final public void run() {
        ...
        try {
            ...
            if (!isStopped()) {
                errmes("fatal error in Prolog engine: " + this, e);
        ...
        if (0 != result) {
            warnmes("error in bg: " + termToString(result));
        }
        stop();
    }
    ...
    public final boolean exportBundle(int cloned) {
        if (null != this.ioBundle || null == this.messageBox) {
            return false;
        }
    ...
    synchronized boolean stats(int op) {
        switch (op) {
            case RUNTIME_2: {
                int time = (int) (currentTimeMillis() - prolog.startTime);
    ...
    private void show_trace() {
        ...
        warnmes("\tAn: R" + LCGET(curInstr) + "=" + An() + ", Ai: R"
                + MCGET(curInstr) + "=" + Ai());
    }
