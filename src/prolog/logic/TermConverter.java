...
import static prolog.logic.Defs.isINTEGER;
...
public class TermConverter implements ITerm, Stateful {
     ...
     * assuming a different Prolog as a client. A slightly faster put_fun(String
    ...
    public int putInt(int i) {
        return heap.pushTerm(INPUT_INT(i));
    }
    ...
    public static final double toDouble(int i1, int i2, int i3) {
        long left = OUTPUT_INT(i1);
        long right = OUTPUT_INT(i2);
        long res = OUTPUT_INT(i3);
        left = left << 32;
        right = right << chop;
    ...
    private int putFloatParts(int i, int dec, int res) {
     ...
     * accomodates the usual bottom-up term building of the WAM and can be used
     * to attach other Prologs with different term representaion as well as to
    ...
    private int putOneObject(Object o) throws SyntaxException {
        int arg = INPUT_INT(atomTable.addObject(o));
    ...
    private int putObjectInternal(Object O) throws PrologException {
        //Interact.println("result="+O);
        int res;
        if (null == O) {
            res = heap.prolog.G_null;
        } else if (O instanceof Integer) {
            res = putInt((int) (Number) O);
        } else if (O instanceof Double) {
            res = putFloat((double) (Number) O);
    ...
    /**
     * Part of the ITerm interface: digests an external Object and returns an
     * int handle to it
     */
    /**
     * part of ITerm interface: builds external terms through the OTerms
     * interface
     *
     * @param xref
     * @return
    ...
    private Object getTermInternal(int xref, OTerm O) throws PrologException {
        if (0 == xref) {
            convertError("putTerm: variable cannot be 0");
        }
        int xval;
        if (isVAR(xref)) {
        ...
        if (isVAR(xval)) {
            ...
            if (0 == arity) {
                ...
                int arg = xref + 1;
                heap.deref(arg);
                if (isINTEGER(heap.xval)) {
                    int handle = OUTPUT_INT(heap.xval);
                ...
                if (null == R) {
                    convertError("bad object handle in getTermInternal(): " + heap.xval);
                }
    ...
    public static String toQuoted(String s) {
        ...
        if ((isUpperCase(a)
                ...
                || '_' == a)) {
            needsq = true;
        } else if (s.equals("!")) { /* without quotes */ } else {
