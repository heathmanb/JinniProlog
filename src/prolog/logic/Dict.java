...
final class Dict implements Stateful {
     ...
     *
     * @param prolog The Prolog instance with which this dictionary is
     * associated.
  ...
  //for(int i=0;i<entries.length;i++) {
     ...
     */
    private void rehash() throws PrologException {
    ...
    private void rehash0() throws PrologException {
        //System.err.println("ENTERING rehash, used:"+dictUsed+
        //        " max:"+dictMax+" low:"+dictLow+" high: "+dictHigh);
        int oldMax = dictMax;
        if (dictUsed < dictLow) {
            dictMax = dictMax >> 1; // shrink to half if 1/8 used; => 25% full
        } else {
            dictMax = dictMax << 1; // double if 1/2 used => 25% full
        }
        ...
        for (int level = 0; level <= Prolog.BBOARDTIME; level++) {
            ...
            for (int j = 0; j < oldMax; j++) {
                ...
                if (null != entries[i]) {
                    throw new SystemException("error in rehash on:" + entry);
                }
                entries[i] = entry;
     ...
     * <p>
     * Implementation Note:
     * <br>This is implemented as a static function to aid inlining by the
     * compiler.
     */
//private final static int hashKeys(int dictMax, int key1, int key2) {
//	return Math.abs((key1 + key2) & (dictMax-1));
//}
    private static int hashKeys(int dictMax, int key1, int key2) {
        //return Math.abs(((key1<<2)^(key1>>11) + (key2<<5)^(key2>>17)) & (dictMax-1));
        return abs(((key1 << 8) + ((key2 << 5) ^ key2)) & (dictMax - 1));
    }
    ...
    final boolean isFree(DictEntry entry) throws TypeException {
        ...
        if (entry.timeStamp < prolog.timeStamp) {
    ...
    /**
     * Create a new dictionary entry. Entries are tagged with the current prolog
     * epoch (time stamp). If an entry already exists then its value is
     * <EM>not</EM> updated.
     *
    ...
    /**
     * For internal use only. Retrieves a DictEntry or null.
     *
     ...
     * compiler.
     */
    private static int hget_internal(DictEntry[] entries, int dictUsed, int pred, int fun) throws ResourceException {
...
// builtins
    ...
    final int do_hget(int k1, int k2, HeapStack heap) throws PrologException {
        int t = hget(k1, k2);
        if (prolog.G_ref == t) {
            t = heap.decodedCopy((EncodedTerm) hget_ref(k1, k2));
        }
        return t;
    }
...
class DictEntry implements Stateful {
  ...
  //private final void setTimeStamp(int timestamp) {
    ...
    public String toString() {
        return "<" + prolog.codeStore.NAME(pred) + "/" + GETARITY(pred) + ","
                + prolog.codeStore.NAME(fun) + "/" + GETARITY(fun) + ">"
                + "==>" + val + ":(" + timeStamp + ")";
    }
}// End class DictEntry
