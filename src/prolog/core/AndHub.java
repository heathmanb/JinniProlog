...
/**
 * An AndHub is a generic AND synchronizer. An array of exactly max slots is
 * first filled up by Producers such that each slot holds exactly one value.
 * Producers are blocked until all values are ready or if they try to reassign a
 * cell that is full. Consumers are blocked until the array is full or if they
 * try to take a value more than once. A typical use is as a data flow primitive
 * for a max argument function. For this purpose a getAll operation is provided
 * that consumes all values atomically, when ready.
 */
