...
/**
 * Provides some utility functions on infinite length numbers.
 */
public class BigMath implements Stateful {
    ...
    /**
     * Converts an n-tuple of big integers to a single big integer as a
     * bijection from Nx...xN to N.
     * @param Tuple
     * @return 
     */
     ...
     * indicating its 0's or 1's locations, depending on selectOnes being false
     * or true
     * @param l
     * @param N
     * @param selectOnes
    ...
    public static Cat bigint2igraph(BigInteger B) {
        ...
        for (int i = 0; i < l; i++) {
            ...
            Integer F = Xs[0].intValue();
            Integer T = Xs[1].intValue();
            C.setProp(F, "x", "a");
            C.setHyper(F, 0);
            C.setProp(T, "x", "a");
            C.setHyper(T, 0);
