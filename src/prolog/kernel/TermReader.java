...
public class TermReader extends TermConverter {
    ...
    private void initParser(String charIn) {
        this.charIn = charIn;
    ...
    private int parse1(String s, boolean warnSingletons) throws PrologException {
        s = fixInput(s);
        if (null == s) {
            return 0;
        }
        ...
        while (e.hasNext()) {
            ...
            int var = ((Integer) varTable.get(key));
            int[] args = {name, var};
    ...
    /**
     * advances on character
     */
    private char advChar() {
        //Prolog.dump(">"+curChar);
     ...
     */
    private void backtrack(int where) {
     ...
     * ********************* LEXICAL ANALYSER ********************
     */
    private void skipWhitespace() throws SyntaxException {
        for (;;) {
            if (eof()) {
                break;
    ...
    static {
        for (String atom_op : atom_ops) {
            opTable.put(atom_op, "atom_op");
        }
        for (String term_op : term_ops) {
            opTable.put(term_op, "term_op");
        }
    }
    ...
    private int cons(int hd, int tl) throws PrologException {
            ...
            if (match_char('(')) {
                ...
                if (first != 0) {
                    ...
                    if (arg != 0) {
                        ...
                        readError("error in compound term");
                    }
                }
                return first;
            } else {
    ...
    private int match_args(int arg, IntStack stack) throws PrologException {
        ...
        if (match_char(')')) {
            ...
            //argvect[arg] = t;
            stack.push(t);
            if (t != 0) //arg = match_args(++arg, argvect);
            {
                arg = match_args(++arg, stack);
            }
        } else {
